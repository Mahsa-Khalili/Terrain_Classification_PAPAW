...
TIME_FEATURES = {'Mean': np.mean, 'Std': np.std, 'Norm': l2norm, 'Max': np.amax, 'Min': np.amin,
				 'RMS': rms, 'ZCR': zcr}
...
TIME_FREQ_FEATURES = {'Mean': np.mean, 'Std': np.std, 'Norm': l2norm, 'Max': np.amax, 'Min': np.amin,
					'RMS': rms, 'ZCR': zcr, 'MSF': msf, 'RMSF': rmsf,
					'FC': fc, 'VF': vf, 'RVF': rvf}
...
class ClIMUDataStream(threading.Timer):
	...
	def __init__(self, dataQueue, runMarker, testSet):
		...
		self.streamFile = pd.read_csv(
			os.path.join(dir_path, "imu_data", "Middle_{}Power{}{}_Module{}.csv".format(testSet[1], testSet[2], testSet[0], IMU)))
		self.streamRow = 0
...
if __name__ == "__main__":
	...
	for testSet in TERRAIN_DICT:
		...
		print("TERRAIN TYPE: {}".format(testSet[1]))
